...
class TelegramAdapter(TelegramRichInboundMixin, BasePlatformAdapter):
    ...
    def _register_handlers(self, app) -> None:
        ...
        # Telegram Bot API 10.1+ Rich Messages (heading, formatted text, list,
        # table, photo caption, links/buttons, etc.) arrive with empty
        # message.text — content lives only in message.api_kwargs["rich_message"]
        # ["blocks"]. Such messages match none of the filters above (no
        # top-level text/media), so PTB would drop them silently. Register a
        # catch-all that forwards them through the text pipeline via
        # _handle_rich_message (rich -> Markdown, media blocks downloaded).
        app.add_handler(TelegramMessageHandler(
            filters.ALL
            & ~filters.COMMAND
            & ~filters.TEXT
            ...
            & ~filters.Sticker.ALL
            & ~filters.LOCATION
            & ~getattr(filters, "VENUE", filters.LOCATION),
